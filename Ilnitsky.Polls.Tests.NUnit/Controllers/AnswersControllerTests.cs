...
namespace Ilnitsky.Polls.Tests.NUnit.Controllers;
...
public class AnswersControllerTests
{
    ...
    {
        Assert.That(_httpContext.Items, Contains.Key("ErrorDetails"));
        var errorDetails = _httpContext.Items["ErrorDetails"] as string;

        Assert.Multiple(() =>
        {
            Assert.That(errorDetails, Does.StartWith(expectedStart));
            Assert.That(errorDetails, Does.Contain(expectedValue));
        });
    }
    ...
    public async Task CreateRespondentAnswer_ReturnsBadRequest_WhenRespondentIdIsMissing()
    {
        ...
        Assert.That(badRequestObjectResult.Value, Is.EqualTo("Некорректный идентификатор респондента!"));
    ...
    public async Task CreateRespondentAnswer_ReturnsBadRequest_WhenRespondentSessionIdIsMissing()
    {
        ...
        Assert.That(badRequestObjectResult.Value, Is.EqualTo("Некорректный идентификатор сессии респондента!"));
    ...
    public async Task CreateRespondentAnswer_CallsHandler_WhenRespondentIdAndSessionAreValid()
    {
        ...
        Assert.That(createdResult.Value, Is.EqualTo("Ответ принят!"));
    }
    ...
    public async Task CreateRespondentAnswer_ReturnsNotFound_WhenHandlerReturnsEntityNotFound()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(problemDetails.Detail, Is.EqualTo("Не найден опрос!"));
        });
