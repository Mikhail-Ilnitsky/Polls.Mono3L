...
public class PollsControllerTests
{
    ...
    public async Task GetPollByIdAsync_ReturnsOk_WhenPollExists()
    {
        ...
        Assert.That(okObjectResult.Value, Is.TypeOf<PollDto>());
        var returnedPollDto = (PollDto)okObjectResult.Value;

        Assert.That(returnedPollDto, Is.EqualTo(expectedPollDto));
    }
    ...
    [Test]
    public void GetPollByIdAsync_ThrowsException_WhenIdIsHackGuid()
    {
        // Arrange
        ...
        var exception = Assert.ThrowsAsync<Exception>(async () =>
            await controller.GetPollByIdAsync(hackId, handlerMock.Object)
        );

        Assert.That(exception.Message, Is.EqualTo("Тестовое исключение!"));

        // v2
        Assert.That(
            async () => await controller.GetPollByIdAsync(hackId, handlerMock.Object),
            Throws
                .TypeOf<Exception>()
                .With.Message.EqualTo("Тестовое исключение!"));
    }
}
